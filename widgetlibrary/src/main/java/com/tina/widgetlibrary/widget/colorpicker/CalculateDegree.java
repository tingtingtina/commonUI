...
public class CalculateDegree {
    ...
    /**
     * @param point
     * @return 获得点所在角度（点与坐标轴原点连线与Y正半轴的逆时针夹角）单位为度数
     */
    public static int getRadianByPosition(Point point) {
        double angle = getAngleByPosEx(point);
        return (int) (angle * (360 / (2 * PI)));
    }

    /**
     * @param point
     * @return 获得点所在角度（点与坐标轴原点连线与Y正半轴的逆时针夹角）单位为弧度
     */
    private static double getAngleByPosEx(Point point) {
